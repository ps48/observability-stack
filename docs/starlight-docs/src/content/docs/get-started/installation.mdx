...
For demos or workshops, enable anonymous authentication so users can access Dashboards without logging in. Set `OPENSEARCH_ANONYMOUS_AUTH_ENABLED=true` in `.env` and restart:
```bash
